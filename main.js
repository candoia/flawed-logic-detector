...
$(window).load(function() {
  ...
  let json = api.boa.run('flawed-logic.boa');
  $('#loading').hide();
  $('#content').show();
  ...
  for(let index in json.If_Count) {
    count++;
    $('#numToShow').append(`<option value="${count}"> ${count} </option>`);
    chartData.push({
        label: index,
        value: json.If_Count[index],
  ...
  $('#app-title').html(`Depth of If-Else Logic on ${instance.repos.name}`);
